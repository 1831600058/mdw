...
from utils import get_accuracy, mixup_data, noisy_arcface_criterion
...
class Trainer:
    def __init__(self, device, mode, m, alpha, epochs=300, class_num=41, lr=1e-4):
        ...
        self.epochs = epochs
        self.alpha = alpha
        self.net = MixData_WeightProcess(num_classes=class_num, mode=mode, use_arcface=True, m=m).to(self.device)
        ...
        self.test_criterion = ASDLoss().to(self.device)
        #self.criterion = BCELoss().to(self.device)
        #self.test_criterion = BCELoss().to(self.device)
        self.mode = mode
            ...
            for _, (x_wavs, x_mels, labels) in tqdm(enumerate(train_loader), total=num_steps):
                ...
                with autocast():
                    if self.mode == 'arcface':
                        ...
                        loss = noisy_arcface_criterion(self.criterion, logits, y_a, y_b, lam)
